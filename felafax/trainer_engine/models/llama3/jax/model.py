...
class Attention(nn.Module):
    ...
    def __call__(
        ...
        outputs = ((attn_output.astype(self.dtype),
                    attn_weights) if output_attentions else
                   (attn_output.astype(self.dtype), ))
        return outputs
...
class LlamaModule(nn.Module):
    ...
    def setup(self):
        ...
        self.wte = nn.Embed(
            ...
            param_dtype=jnp.
            bfloat16,  # Embedding always use bfloat16 or float32
        )
    ...
    def __call__(
        ...
        if output_hidden_states:
            all_hidden_states = tuple(
                h.astype(self.dtype)
                for h in outputs[1]) + (hidden_states.astype(self.dtype), )
            outputs = (hidden_states.astype(
                self.dtype), all_hidden_states) + outputs[2:]
        else:
