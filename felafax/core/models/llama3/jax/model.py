...
class Attention(nn.Module):
    ...
    def __call__(
        ...
        xq = jax_utils.apply_sharding_constraint(xq, PS(("dp", "fsdp"), None, "mp"))
        xk = jax_utils.apply_sharding_constraint(xk, PS(("dp", "fsdp"), None, "mp"))
        xv = jax_utils.apply_sharding_constraint(xv, PS(("dp", "fsdp"), None, "mp"))
        ...
        outputs = (
            (attn_output.astype(self.dtype), attn_weights)
            if output_attentions
            else (attn_output.astype(self.dtype),)
        )
        return outputs
...
class LlamaModule(nn.Module):
    ...
    def __call__(
        ...
        if output_hidden_states:
            all_hidden_states = tuple(h.astype(self.dtype) for h in outputs[1]) + (
                hidden_states.astype(self.dtype),
            )
            outputs = (hidden_states.astype(self.dtype), all_hidden_states) + outputs[
                2:
            ]
        else:
