...
def test_batch_shapes():
    ...
    for key in ["input_ids", "labels"]:
        assert (train_batch[key].shape[0] == data_module.batch_size
                ), f"Unexpected batch size for train_batch[{key}]"
        assert (train_batch[key].shape[1] <= data_module.max_seq_length
...
def test_special_tokens_with_large_seq_length():
    ...
    tokenizer, data_module = setup_alpaca_dataset(batch_size=1,
    ...
    assert (tokenizer.bos_token_id
            in sample_ids), "BOS token not found in sequence"
